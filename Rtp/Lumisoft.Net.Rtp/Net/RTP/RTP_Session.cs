...
namespace LumiSoft.Net.RTP
{
    ...
    public class RTP_Session : IDisposable
    {
        ...
        internal RTP_Session(RTP_MultimediaSession session, RTP_Clock clock, RTP_Address sourceEP = null)
        {
            ...
            if (sourceEP == null)
            {//sender
                m_pRtpSocket = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);

                m_pRtcpSocket = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
            }
            else
            {//receiver
                m_pRtpSocket = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
                m_pRtpSocket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReuseAddress, 1);
                m_pRtpSocket.Bind(m_pLocalEP.RtpEP);
                m_pRtpSocket.SetSocketOption(SocketOptionLevel.IP, SocketOptionName.AddMembership, new MulticastOption(sourceEP.IP, IPAddress.Any));

                m_pRtcpSocket = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
                m_pRtcpSocket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReuseAddress, 1);
                m_pRtcpSocket.Bind(m_pLocalEP.RtcpEP);
                m_pRtcpSocket.SetSocketOption(SocketOptionLevel.IP, SocketOptionName.AddMembership, new MulticastOption(sourceEP.IP, IPAddress.Any));
            }
        ...
        private RTP_Address m_pSourceEP = null;
